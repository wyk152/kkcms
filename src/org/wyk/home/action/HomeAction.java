...
public class HomeAction extends ActionSupport {
	...
	public String execute(){
		...
		List<Info> infoList = infoService.findObjectsList(queryHelper1, 1, 5).getItems();
		...
		List<Complain> complainList = complainService.findObjectsList(queryHelper2, 1, 6).getItems();
	...
	public void getUserJson(){
		try {
			...
			if(StringUtil.isNotBlank(dept)){
				...
				//2、根据部门查询用户列表
				List<User> userList = userService.findObjectsList(queryHelper);
				//创建Json对象
	...
	public String getUserJson2(){
		try {
			...
			if(StringUtil.isNotBlank(dept)){
				...
				return_map.put("userList", userService.findObjectsList(queryHelper));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
